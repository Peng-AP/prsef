...
def updateModels():
    print("AAAA")
    classModel.set('')
    classModelSelection['menu'].delete(0, 'end')
    modelType.set(val)
    type = modelType.get()
    for model in glob.glob(script_dir + f"/{type}models/*"):
...
modelTypeSelection['menu'].add_command(label="32x32",command = updateModels)
modelTypeSelection['menu'].add_command(label="72x72",command = updateModels)

modelLabel.grid(column = 0, row = 0)
modelTypeSelection.grid(column = 0, row = 1)
classLabel.grid(column = 0, row = 2)
classModelSelection.grid(column = 0, row = 3)
